...
static DEF_INITIAL_LOONG_SIZE: u16 = 3;
...
impl Options {
  ...
  pub fn initial_loong_size(mut self, val: u16) -> Self {
    self.initial_loong_size = Some(val);
    self
  }
...
pub(crate) struct InnerCfg {
  ...
  pub(crate) initial_loong_size: u16,
...
impl InnerCfg {
  pub(crate) fn from_options(opts: &Options) -> Self {
    let loong_size = if let Some(s) = opts.initial_loong_size {
      if s < 3 {
        3
      } else {
        s
