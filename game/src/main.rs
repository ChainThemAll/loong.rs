...
static BOARD_DIM_X: u16 = 40;
static BOARD_DIM_Y: u16 = 40;
...
use loong_ctrl::{Direction, LoongCtrl, LoongCtrlErr, LoongCtrlOptions};
use view::colors;

pub struct App<'a> {
  gl: GlGraphics,
  loong_ctrl: LoongCtrl,
  score: u64,
...
impl<'a> App<'a> {
  fn handle_key_press(&mut self, key: Key) {
    match key {
      Key::Right => {
        self.loong_ctrl.direction_to(Direction::Right).unwrap();
      }
      Key::Left => {
        self.loong_ctrl.direction_to(Direction::Left).unwrap();
      }
      Key::Up => {
        self.loong_ctrl.direction_to(Direction::Top).unwrap();
      }
      Key::Down => {
        self.loong_ctrl.direction_to(Direction::Bottom).unwrap();
      }
      Key::Space => {
  ...
  fn restart_game(&mut self) {
    self.score = 0;
    self.timer = timer::Timer::new(100);
    self.loong_ctrl.restart().unwrap();
  ...
  fn render(&mut self, args: &RenderArgs) {
    ...
    let state = self.loong_ctrl.get_full_state_reversed_y();

    let food_sprite = self.sprites.apple();

    self.gl.draw(viewport, |c, gl| {
      let r = Rectangle::new(*colors::BLACK).border(Border {
  ...
  fn update(&mut self, _args: &UpdateArgs) {
    match self.loong_ctrl.next_tick() {
      Ok(is_ate) => {
        if is_ate {
          self.score += 1;
      ...
      Err(e) => match e {
        LoongCtrlErr::LoongAteItself | LoongCtrlErr::LoongHitTheWall => {
